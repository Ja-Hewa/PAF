...
*--------------------------------------------------
* PAF - L24h KHK (NORAH) - leere Datensatz
*--------------------------------------------------
gen id = . 
gen gPAF = .
gen glPAF = .
...
*l24 oder lden
gen RR = exp(ln(1.024)*(lden-40)/10) 
gen LC = exp(ln(1.016)*(lden-40)/10)
gen UC = exp(ln(1.033)*(lden-40)/10)
...
append using DALY_KHK.dta, keep(id Gemeinde gPAF glPAF guPAF ) force
save DALY_KHK.dta, replace

}
*

 use DALY_KHK.dta, clear
